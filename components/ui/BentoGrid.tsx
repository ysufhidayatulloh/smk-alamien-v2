...
export const BentoGridItem = ({
  ...
  const handleCopy = () => {
    navigator.clipboard.writeText("https://smkalamienkdr.sch.id/");
          ...
          {id === 3 && (
            <div className="flex gap-1 lg:gap-5 w-fit absolute -right-3 lg:-right-2">
              <div className="flex flex-col gap-3 lg:gap-8">
                {["Academic", "Achievement", "Ecosystem"].map((item) => (
                  <span
                    key={item}
              ...
              <div className="flex flex-col gap-3 lg:gap-8">
                <span className="py-4 px-3 rounded-lg text-center bg-[#10132E]"></span>
                {["Discipline", "Morals", "Modern"].map((item) => (
                  <span
                    key={item}
                    className="py-2 lg:py4 lg:px-3 px-3 text-xs lg:text-base opacity-50 lg:opacity-100 rounded-lg text-center bg-[#10132E]"
                  >
          ...
          {id === 6 && (
            <div className="mt-5 relative">
              ...
              <a href="https://s.id/PPDBSMKAAM24-25" target="_blank">
                <MagicButton
                  title={copied ? "Yes, you have joined !!" : "Join Us Now !!!"}
                  icon={<MdJoinFull />}
                  position="left"
